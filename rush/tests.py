...
class RegisterViewTestCase(BaseTest):
    ...
    def test_register_not_allowed_methods(self, method):
        response = self.client.generic(method=method, path=f"/api/register/")
    ...
    def test_register_user(self):
        ...
        response = self.client.post(path=f"/api/register/", data=new_user_data)
...
class AuthenticationTestCase(BaseTest):
    ...
    def test_cant_auth_with_empty_password(self):
        response = self.client.post(
            path=f"/api/token/", data={"username": "invalid_username"}
        )
    ...
    def test_success_get_user_access_token(self):
        ...
        response = self.client.post(
            path=f"/api/token/", data={"username": "setupuser", "password": "pass"}
        )
